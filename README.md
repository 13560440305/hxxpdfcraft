...
<div align="center">
  <img src="public/images/logo.png" alt="hxxpdf Logo" width="120" height="120" />
  <h1>hxxpdf</h1>
  <p>
...
**hxxpdf** is a comprehensive suite of PDF tools designed for privacy and performance. Unlike many online converters, hxxpdf processes your files entirely within your browser using WebAssembly technology. Your documents **never** leave your device, ensuring maximum security for your sensitive data.
...
- **🛠�?Comprehensive Toolset**: Over 80+ tools to handle any PDF task.
- **🎨 Modern UI**: Clean, accessible, and responsive design built with Tailwind CSS.
- **🌐 Multi-language**: Supports English, Spanish, French, German, Portuguese, Japanese, Korean, Chinese, and more.
...
hxxpdf includes a powerful **visual workflow editor** that allows you to chain multiple PDF operations together, creating automated processing pipelines.
...
Navigate to `/workflow` or click on "Workflow" in the navigation menu.
...
hxxpdf provides both pre-built Docker images and Docker Compose for flexible deployment options.

#### Option 1: Use Pre-built Image (Recommended)

The easiest way to run hxxpdf is using our pre-built image from GitHub Container Registry:
...
# Run the container
docker run -d -p 8080:80 --name hxxpdf ghcr.io/pdfcrafttool/pdfcraft:latest
```
...
hxxpdf supports deployment under a subpath (e.g., `https://your-domain.com/pdfcraft/`). To enable this, you must specify the base path during the build process.

**Using Docker Build:**
```bash
docker build --build-arg BASE_PATH=/pdfcraft -t hxxpdf .
```
...
hxxpdf is configured for static export (`output: 'export'`), which means it can be deployed to any service that supports static website hosting without requiring a Node.js server.
...
hxxpdf stands on the shoulders of giants. We gratefully acknowledge [BentoPDF](https://github.com/alam00000/bentopdf) for their pioneering work in privacy-first, client-side PDF tools.

Their project served as a significant inspiration and reference for our core logic. While hxxpdf has been re-engineered for the Next.js ecosystem and extends functionality with unique features like the *Workflow Editor*, we deeply respect the foundation laid by the BentoPDF team.
...
<div align="center">
  Built with ❤️ by the hxxpdf team
</div>
